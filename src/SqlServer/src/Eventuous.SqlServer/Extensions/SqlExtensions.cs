...
static class SqlExtensions {
    extension(SqlCommand command) {
        internal SqlCommand AddPersistedEvent(string parameterName, IEnumerable<NewPersistedEvent> persistedEvents) {
        ...
        internal SqlParameter AddOutputParameter(string parameterName, SqlDbType sqlDbType)
            => command.Parameters.Add(new() { ParameterName = parameterName, SqlDbType = sqlDbType, Direction = ParameterDirection.Output });

        internal SqlCommand AddOutput(string parameterName, SqlDbType sqlDbType) {
            command.Parameters.Add(new() { ParameterName = parameterName, SqlDbType = sqlDbType, Direction = ParameterDirection.Output });

            return command;
        }

        internal SqlCommand Add(string parameterName, SqlDbType sqlDbType, object value) {
    ...
    extension(SqlConnection connection) {
        internal SqlCommand GetTextCommand(string sql) {
        ...
        internal SqlCommand GetStoredProcCommand(string storedProcName, SqlTransaction? transaction = null) {
            var cmd = connection.CreateCommand();
