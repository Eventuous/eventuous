...
public static class TypeMapperExtensions {
    ...
    extension(ITypeMapper typeMapper) {
        ...
        public string GetTypeName<T>(bool fail = true) => typeMapper.GetTypeNameByType(typeof(T), fail);

        public bool TryGetTypeName<T>([NotNullWhen(true)] out string? typeName) => typeMapper.TryGetTypeName(typeof(T), out typeName);
