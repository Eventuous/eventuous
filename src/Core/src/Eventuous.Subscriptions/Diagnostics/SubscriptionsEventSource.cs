...
public class SubscriptionsEventSource : EventSource {
    ...
    public void CheckpointAlreadyCommitted(string id, CommitPosition checkpoint) {
        if (IsEnabled(EventLevel.Verbose, Keywords.Checkpoints))
            CheckpointAlreadyCommitted(id, checkpoint.Position);
    }
    ...
    [Event(CheckpointAlreadyCommittedId,
        Message = "Checkpoint already committed {0}:{1}",
    ...
    [Event(CheckpointLastCommitGapId,
        Message = "Last commit position {0}:{1} is behind latest position {2}:{3}",
        Level = EventLevel.Verbose,
        Keywords = Keywords.Checkpoints
    )]
    void CheckpointLastCommitGap(
        ulong lastCommitSequence,
        ulong lastCommitPosition,
        ulong latestSequence,
        ulong latestPosition
    )
        => WriteEvent(CheckpointLastCommitGapId,
            lastCommitSequence,
            lastCommitPosition,
            latestSequence,
            latestPosition
    ...
    [Event(CheckpointGapDetectedId,
        Message = "Gap detected in checkpoint between {0}:{1} and {2}:{3}",
        Level = EventLevel.Verbose,
        Keywords = Keywords.Checkpoints
    )]
    void CheckpointGapDetected(
        ulong beforeSequence,
        ulong beforePosition,
        ulong afterSequence,
        ulong afterPosition
    )
