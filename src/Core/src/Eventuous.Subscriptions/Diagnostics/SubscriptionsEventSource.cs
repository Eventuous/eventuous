...
public class SubscriptionsEventSource : EventSource {
    ...
    [NonEvent]
    public void MessageTypeNotRegistered<T>()
        => MessageTypeNotRegistered(typeof(T).Name);
    ...
    void MetricCollectionFailed(string metric, string exception)
    ...
    void MessageTypeNotRegistered(string messageType)
    ...
    void CheckpointAlreadyCommitted(string id, ulong position)
        => WriteEvent(CheckpointAlreadyCommittedId, id, position);

    [Event(CheckpointLastCommitGapId, Message = "Last commit position {0}:{1} is behind latest {2}:{3}", Level = EventLevel.Verbose, Keywords = Keywords.Checkpoints)]
    ...
    [Event(CheckpointLastCommitDuplicateId, Message = "Last commit position {0}:{1} equals the latest", Level = EventLevel.Warning, Keywords = Keywords.Checkpoints)]
    ...
    [Event(CheckpointSequenceInvalidHeadId, Message = "Last commit position {0}:{1} sequence is not zero", Level = EventLevel.Verbose, Keywords = Keywords.Checkpoints)]
    ...
    [Event(CheckpointGapDetectedId, Message = "Gap detected in checkpoint between {0}:{1} and {2}:{3}", Level = EventLevel.Verbose, Keywords = Keywords.Checkpoints)]
