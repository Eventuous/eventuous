...
class BatchedChannelWorker<T>(Channel<T> channel, ProcessElement<IReadOnlyList<T>> processor, int maxCount, TimeSpan maxTime)
    : ChannelWorkerBase<T>(channel, token => channel.ReadBatches(processor, maxCount, maxTime, token), 1);
