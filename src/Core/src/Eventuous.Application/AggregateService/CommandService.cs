...
public abstract partial class CommandService<TAggregate, TState, TId>(
    ...
    /// </summary>
    /// <param name="command">Command to execute</param>
    /// <param name="amend">Function to add additional information to the event before it's stored.</param>
    /// <param name="cancellationToken">Cancellation token</param>
    /// <returns><see cref="Result{TState}"/> of the execution</returns>
    /// <exception cref="Exceptions.CommandHandlerNotFound{TCommand}"></exception>
    public async Task<Result<TState>> Handle<TCommand>(TCommand command, AmendEvent amend, CancellationToken cancellationToken) where TCommand : class {
        ...
        try {
            ...
            var storeResult = await store.Store(GetAggregateStreamName(), result, amend, cancellationToken).NoContext();
