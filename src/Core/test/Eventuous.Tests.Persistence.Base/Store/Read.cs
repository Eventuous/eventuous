...
public abstract class StoreReadTests<T> where T : StoreFixtureBase {
    ...
    [Test]
    [Category("Store")]
    public async Task ShouldReturnEmptyWhenReadingBackwardsFromBeyondStreamEnd(CancellationToken cancellationToken) {
        object[] events     = _fixture.CreateEvents(10).ToArray();
        var      streamName = Helpers.GetStreamName();
        await _fixture.AppendEvents(streamName, events, ExpectedStreamVersion.NoStream);
    ...
    [Test]
    [Category("Store")]
    public async Task ShouldReturnEmptyWhenReadingBackwardsFromNegativePosition(CancellationToken cancellationToken) {
        object[] events     = _fixture.CreateEvents(10).ToArray();
        var      streamName = Helpers.GetStreamName();
        await _fixture.AppendEvents(streamName, events, ExpectedStreamVersion.NoStream);

        // Try to read from negative position
        var result = await _fixture.EventStore.ReadEventsBackwards(streamName, new(-1), 5, true, cancellationToken);
