...
public class OnlyArchive {
    readonly TieredEventReader _tieredEventReader;
    ...
    public OnlyArchive(IElasticClient elasticClient, EventStoreClient eventStoreClient) {
        ...
        _tieredEventReader = new(_esdbEventStore, elasticEventStore);
    }

    public async Task Execute() {
        const string bookingId = "deea3663-17c0-45a6-86b2-70c66fd407fd";

        var service = new ThrowingCommandService<Booking, BookingState, BookingId>(new BookingService(_tieredEventReader, _esdbEventStore));
