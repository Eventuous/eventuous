...
public sealed class SpyglassGenerator : IIncrementalGenerator {
            ...
            for (var bt = type.BaseType; bt is not null; bt = bt.BaseType) {
                if (bt.OriginalDefinition.ToDisplayString() == StateFqn && bt.TypeArguments.Length == 1) {
                    ...
                    if (!excludeFqns.Contains(stateFqn)) {
                        builder.Add(new StateCandidate(stateFqn, type.Name));
                    }
    ...
    static void ProcessType(INamedTypeSymbol type, ImmutableArray<AggregateCandidate>.Builder builder) {
        if (!type.IsAbstract && type.TypeKind == TypeKind.Class) {
            ...
            if (stateType is not null) {
                ...
                builder.Add(new AggregateCandidate(aggregateFqn, type.Name, stateFqn, stateType.Name, methods));
            }
        }
    ...
    readonly struct StateCandidate : IEquatable<StateCandidate> {
        public readonly string StateFqn;
        public readonly string StateSimpleName;

        public StateCandidate(string stateFqn, string stateSimpleName) {
            StateFqn        = stateFqn;
            StateSimpleName = stateSimpleName;
        }

        public bool Equals(StateCandidate other) => StateFqn == other.StateFqn;

        public override bool Equals(object obj) => obj is StateCandidate other && Equals(other);

        public override int GetHashCode() => StateFqn?.GetHashCode() ?? 0;
    }

    readonly struct AggregateCandidate : IEquatable<AggregateCandidate> {
        public readonly string                 AggregateFqn;
        public readonly string                 AggregateSimpleName;
        public readonly string                 StateFqn;
        public readonly string                 StateSimpleName;
        public readonly ImmutableArray<string> Methods;

        public AggregateCandidate(string aggregateFqn, string aggregateSimpleName, string stateFqn, string stateSimpleName, ImmutableArray<string> methods) {
            AggregateFqn        = aggregateFqn;
            AggregateSimpleName = aggregateSimpleName;
            StateFqn            = stateFqn;
            StateSimpleName     = stateSimpleName;
            Methods             = methods;
        }

        public bool Equals(AggregateCandidate other)
            => AggregateFqn == other.AggregateFqn;

        public override bool Equals(object obj)
            => obj is AggregateCandidate other && Equals(other);
