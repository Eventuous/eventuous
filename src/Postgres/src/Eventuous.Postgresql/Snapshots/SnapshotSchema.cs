...
public class SnapshotSchema(string schema = SnapshotSchema.DefaultSchema) {
    ...
    public string WriteSnapshot
        => $"INSERT INTO {schema}.snapshots (stream_name, revision, event_type, json_data) VALUES (@stream_name, @revision, @event_type, @json_data) ON CONFLICT (stream_name) DO UPDATE SET revision = @revision, event_type = @event_type, json_data = @json_data, created = now() at time zone 'utc'";
