...
| `T Match<T>(Func<Ok, T> matchOk, Func<Error, T> matchError)` | Function that can be used for pattern matching success and error if the output has the same type. Eventuous uses this function for producing `IResult` and `ActionResult` in HTTP API extensions. |
