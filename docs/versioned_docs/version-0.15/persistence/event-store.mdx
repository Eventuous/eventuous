...
It's possible to make some changes in events before they get persisted using the `AmendEvent` function provided as `amendEvent` argument. The `AmendEvent` type is equivalent to `Func<NewStreamEvent, NewStreamEvent>`.
