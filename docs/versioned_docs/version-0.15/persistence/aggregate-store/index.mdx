...
Since version **0.15**, Eventuous doesn't use _Aggregate store_ internally. All persistence operations in command services are using [Event store](../event-store.md). The following elements are marked obsolete:
...
builder.Services
    .AddEventStoreClient(connectionString)
    .AddAggregateStore<EsdbEventStore>();
```

The `AddAggregateStore` extension is available in the `Eventuous.Extensions.DependencyInjection` NuGet package.

:::note
Make sure to read about [events serialization](../serialisation.md).
:::
...
```csharp title="Program.cs"
builder.Services
    .AddEventStore(new MyEventStore(...))
...
:::info Deprecation notice
Since version **0.15**, aggregate store with archive is superseded by [tiered event store](../event-store.md#event-store-with-archive).
:::
